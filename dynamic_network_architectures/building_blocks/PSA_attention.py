...
class SEWeightModule3d(nn.Module):
    ...
    def __init__(self, channels, reduction=16):
        ...
        hidden_ch = max(channels // reduction, 1)  # ✅保证至少1通道
        self.fc1 = nn.Conv3d(channels, hidden_ch, kernel_size=1, padding=0)
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = nn.Conv3d(hidden_ch, channels, kernel_size=1, padding=0)
        self.sigmoid = nn.Sigmoid()
    #def __init__(self, channels, reduction=16):
    #    super(SEWeightModule3d, self).__init__()
    #    self.avg_pool = nn.AdaptiveAvgPool3d(1)
